			...
			{#each $notesAndConversations as noteOrConversation}
				{#if 'createdAt' in noteOrConversation}
					<a
						...
						class="wrapper-sidebar-element "
						><Document /> <div style="text-overflow: ellipsis; width: 11em; white-space: nowrap; overflow: hidden;">{noteOrConversation.title}</div></a
					>
				{:else}
					<a
						href="/ai-{noteOrConversation.id}"
						class="wrapper-sidebar-element "
						><Chatbubble /> <div style="text-overflow: ellipsis; width: 11em; white-space: nowrap; overflow: hidden;">{noteOrConversation.title}</div></a
					>
				{/if}
			{/each}
		{:else if $conversation}
			...
			{#if $conversationMessages}
				{#each $conversationMessages as message}
					<a href="#navigator-{message.id}"
						class="capitalize wrapper-sidebar-element"
						><div style="text-overflow: ellipsis; width: 13em; white-space: nowrap; overflow: hidden;">{message.role.trimStart()}: {message.content}</div></a
					>
				{/each}
			{/if}
		{/if}
