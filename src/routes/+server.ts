...
export async function POST({ request }: { request: Request }) {
	...
	if(!file) {
		...
		if(typeof databaseNote == "string" || typeof databaseConversation == "string") return new Response('400 Bad Request', { status: 400 })
        //@ts-ignore
		return new Response(JSON.stringify([...databaseNote, ...databaseConversation]), {headers: {"Content-Type": 'application/json'}})
	}
	else if(file.startsWith("ai")) {
		let databaseConversation = await getConversation(parseInt(file.split("ai-")[1]), token)
		if(typeof databaseConversation == "string") return new Response('400 Bad Request', { status: 400 })
        return new Response(JSON.stringify(databaseConversation), {headers: {"Content-Type": 'application/json'}})
	}
	else {
        ...
        return new Response(JSON.stringify(databaseNote), {headers: {"Content-Type": 'application/json'}})
	}
}